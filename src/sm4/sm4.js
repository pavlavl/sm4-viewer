...
function sm4_setkey(SK, key) {
  ...
  Statuses.setStatus(
    "Получаем 32 раундовых ключа каждый длиной 32 бита",
    SK.join(" ")
  );
...
function sm4_rounds(sk, input, output) {
  ...
  Statuses.setStatus(
    "Разбиваем полученынй блок сообщения на 4 слова длиной 32 бита",
    ulbuf.join(" ")
  );
  ...
  while (i < 32) {
    ...
    Statuses.setStatus(`Раунд N${i + 1}:`, newbuf);
  ...
  Statuses.setStatus("Объединяем полученные слова в блок данных", output);
}
...
function sm4_crypt_ecb(ctx, input) {
  ...
  for (var t = 0; t < length; t += 16) {
    ...
    Statuses.setStatus(
      `Рвзбиваем входное сообщение на блоки по 16 символов (блок номер ${
        t + 1
      }):`,
      inn.join(" ")
    );
